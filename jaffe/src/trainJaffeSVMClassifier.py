...
import pickle
import os
from sklearn.decomposition import PCA #it compresses the amount of features my recoginizing the most important patterns, im using this for the jaffe dataset as there are more features than samples, leading to overfittng if run on SVM
...
def loadHogFeatures():
    inputDir = os.path.join(jaffeRootDir, 'results', 'jaffeTrainBalancedHOG')
...
def main():
    ...
    # #adjustment to improve accuracy on jaffe dataset
    # featurePolice = PCA(n_components=42, random_state=42)
    # xTrain = featurePolice.fit_transform(xTrain) #learn the patterns from the training data, apply them.
    # xTest = featurePolice.transform(xTest) # use the learnt patterns from the training data.
    
    # print(f"After PCA - Training shape: {xTrain.shape}")
    # print(f"After PCA - Test shape: {xTest.shape}")
    ...
    outputDir = os.path.join(jaffeRootDir, 'results', 'jaffe_svm_model')
