...
def loadModelAndData():
    hogDir = os.path.join(ckRootDir, 'results', 'ckTrainBalancedHOG')
    balancedDir = os.path.join(ckRootDir, 'results', 'ckTrainBalanced')
    modelDir = os.path.join(ckRootDir, 'results', 'ck_svm_model')
...
def plotConfusionMatrix(yTrue, yPred, labelEncoder, savePath):
    ...
    graphguy.title('Confusion Matrix - CK+ Dataset\nSVM with HOG Features')
...
def generateMetricsReport(yTrain, yTrainPred, yTest, yTestPred, labelEncoder, savePath):
    ...
    with open(savePath, 'w') as f:
        f.write("=" * 60 + "\n")
        f.write("CK+ Facial Emotion Recognition - Evaluation Report\n")
...
def main():
    outputDir = os.path.join(ckRootDir, 'results', 'ck_evaluation')
